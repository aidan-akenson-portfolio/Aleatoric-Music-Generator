...
class Scale():
    ...
    def _generate_scale(self):
        ...
        for i in range(self._scale_len - 1):
            ...
            # Generate the note
            new_note = 0
            while new_note in self._notes:
                new_note = random.randrange(1, 11)
        ...
        for i in range(1, self._scale_len):
            ...
            while (
                self._notes[i] - 1 in self._notes and self._notes[i] + 1 in self._notes
                ) or (
                self._notes[i] - 1 in self._notes and self._notes[i] - 2 in self._notes
                ) or (
                self._notes[i] + 1 in self._notes and self._notes[i] + 2 in self._notes
                ): 

                # Generate a candidate note not already in the list
                candidate = random.randrange(1, 11)
                while candidate in self._notes:
                    candidate = random.randrange(1, 11)
                self._notes[i] = candidate
                ...
                if search_attempts >= (11 - self._scale_len):
                    ...
                    while (self._notes[note_mod_pos] + offset) in self._notes:
                        note_mod_pos = random.randrange(1, self._scale_len)
                        offset = random.choice([-1, 1])
                    self._notes[note_mod_pos] += offset
                    search_attempts = 0
        ...
        num_thirds = 0
        for n in self._notes:
            if (n + 3) % 12 in self._notes or (n + 4) % 12 in self._notes:
                num_thirds += 1
        
        if num_thirds < 3:
            # Always add either the major or minor third up from the root for simplicity
            offset = random.choice([0,1])
            if 8 + offset not in self._notes:
                self._notes.append(3 + offset)
            else:
                self._notes.append(3) if offset else self._notes.append(9)
            
            # If more are required, generate random ones
            if num_thirds < 2:
                self._notes.sort()
                new_third = (random.choice(self._notes[1:]) + random.choice([3, 4])) % 12
                while new_third in self._notes:
                    new_third = (random.choice(self._notes[1:]) + random.choice([3, 4])) % 12
                self._notes.append(new_third)
            if num_thirds < 1:
                self._notes.sort()
                new_third = (random.choice(self._notes[1:]) + random.choice([3, 4])) % 12
                while new_third in self._notes:
        ...
        num_sixths = 0
        for n in self._notes:
            if (n + 8) % 12 in self._notes or (n + 49) % 12 in self._notes:
                num_sixths += 1
        ...
        if num_sixths < 3:
            ...
            offset = random.choice([0,1])
            if 8 + offset not in self._notes:
                self._notes.append(8 + offset)
            else:
                self._notes.append(8) if offset else self._notes.append(9)
            
            # If more are required, generate random ones
            if num_sixths < 2:
                self._notes.sort()
                new_sixth = (random.choice(self._notes[1:]) + random.choice([8, 9])) % 12
                while new_sixth in self._notes:
                    new_sixth = (random.choice(self._notes[1:]) + random.choice([8, 9])) % 12
                self._notes.append(new_sixth)
            if num_sixths < 1:
                self._notes.sort()
                new_sixth = (random.choice(self._notes[1:]) + random.choice([8, 9])) % 12
                while new_sixth in self._notes:
        ...
        for n in self._notes:
            ...
            if ((((n + 3) % 12) in self._notes) or (((n + 4) % 12) in self._notes)) and (
                ((n + 7) % 12) in self._notes):
                ...
                if (((n + 7) % 12) in self._notes) and (
                    (((n + 7 + 3) % 12) in self._notes) or (((n + 7 + 4) % 12) in self._notes)) and (
                    ((n + 7 + 7) % 12) in self._notes):
                    likely_root = n

        # Rotate if needed
        if likely_root != 0:
            for i in range(self._scale_len):
                self._notes[i] -= likely_root
                if self._notes[i] < 0:
                    self._notes[i] = 12 + self._notes[i]
